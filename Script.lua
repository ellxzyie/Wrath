...
function GiveGuns()
    ...
    -- Check if the player has AK47 in the backpack 
    local hasAK47 = false
...
local SavedOldTeam
local SavedSpeed
local function LocalPlayer7434sRapidFire()
local Players = game:GetService("Players")
...
local YieldFunctionDelayed = 0.8
...
MetamethodHook = hookmetamethod(game, "__namecall", function(...)
	...
	if Method == "FireServer" and Arguaments[1] == FakeShootEvent and enable then
...
LocalPlayer.Chatted:Connect(function(Message)
	if string.lower(Message) == ".unfastfire" or string.lower(Message) == ".unfirerate" and enable then
		enable = false
...
local function characterAdded()
    if diedpos then
        ...
        repeat task.wait() until game.Players.LocalPlayer.Backpack:FindFirstChild("Remington 870") and game.Players.LocalPlayer.Backpack:FindFirstChild("M9") and game.Players.LocalPlayer.Backpack:FindFirstChild("AK-47")
...
while States.ForceField do
    ...
    elseif player.TeamColor == BrickColor.new("Bright blue") then
        ...
        repeat
        wait(0.01)
        until game.Players.LocalPlayer.TeamColor == BrickColor.new("Bright blue") and game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart")
        LoadPos()
        Notify("Success", " Enabled RapidFire. (Type .unfastfire/.unfirerate to disable)", 2);
    end;
...
spawn(autodestroyGUI)
    ...
    if CMD("lag") then
        ...
        local originalPosition = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
        -- Check if the player has Remington 870 in the backpack before moving
        local hasRemington870e = false
        for _,item in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
            if item.Name == "Remington 870" then
                hasRemington870e = true
                break
            end
        end
    ...
    if CMD("shutdown") or CMD("allcrash") then
        GiveGuns()
...
repeat task.wait() until backpack:FindFirstChild("M9") or player.Character:FindFirstChild("M9")

game:GetService("ReplicatedStorage").ReloadEvent:FireServer(backpack:FindFirstChild("M9") or player.Character:FindFirstChild("M9"))

while task.wait() do
    local Gun = backpack:FindFirstChild("M9") or player.Character:FindFirstChild("M9")
    local Humanoid = player.Character:FindFirstChildOfClass("Humanoid")
    if not Gun or not Humanoid or Humanoid.Health < 1 then
...
coroutine.wrap(LagServer)(200);
    Notify("Success", "Attempting to shutdown server with all 3 crash methods.", 2);
    end;
    ...
    if CMD("ad") or CMD("advertise") or CMD("script") or CMD("flex") then
        Chat("Bible admin is better than your script! Made by GOD himself which has 609 commands and more! Get bible admin by typing /pray");
    end
    if CMD("nspawn") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player then
...
local wh = 'https://discord.com/api/webhooks/nowebhooks4u'
